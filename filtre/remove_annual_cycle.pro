PRO remove_mean_cycle, st, sta, stm, std

IF (N_PARAMS() EQ 0) THEN BEGIN
 print, 'UTILISATION:
 print, 'remove_annual_cycle, st,sta, stm, std'
 print, ''
...
sta[KY].jul=JULDAY(1,1,I,0,0,0) ;c MOYENNE DATE AU 1er JANV de l'ANNEE EN COURS
    sta[KY].val=MEAN(st[WHERE(year EQ I)].val,/NAN)-MEAN(st.val,/NAN)
    KY=KY+1
       ...
       stm[KM].jul=JULDAY(M,15,I,0,0,0) ;c MOYENNE DATE AU 15 DU MOIS EN COURS
            ...
            stm[KM].val=MEAN(st[IM].val,/NAN)-MEAN(st[WHERE(month EQ M)].val,/NAN)
            ENDIF ELSE BEGIN
	    ...
	    std[KD].jul=JULDAY(M,D,I,0,0,0)
               ...
               IF (count GE 1) THEN BEGIN
               std[KD].val=MEAN(st[ID].val,/NAN)-MEAN(st[WHERE((month EQ M) AND (day EQ D))].val,/NAN)
               ENDIF ELSE BEGIN
