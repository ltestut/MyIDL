...
function cmset_op_uniq, a
  ...
  if ct GT 0 then return, ii(wh)     ;pb with sorting tides waves 2MS6 etc ..
  
  return, 0L
end

function cmset_op, a, op0, b, not1=not1, not2=not2, count=count, keep_order=keep_order, $
  ...
  if tp1 GE 4 AND tp1 LE 9 then begin
      ...
      case op of 
          ...
          'AND': begin
              ...
              bi = cmset_op_uniq(b) & nb = n_elements(bi)
              ui = [ai, bi+n1]
              uu = [a,b]    & uo=uu & uu = uu(ui)  ;; Raw union...+ keep original order (ajout LT le 04/01/2012)
              us = sort(uu) & uu = uu(us)  ;; ...and sort     
              ...
              if NOT keyword_set(not1) AND NOT keyword_set(not2) then begin
                  ...
                  ido = INTARR(N_ELEMENTS(uo))        
                  for i=0,N_ELEMENTS(uo)-1 do ido[i]=(where(uo[i] EQ uu(wh), cp2))(0)
                  if count EQ 0 then return, -1L
                  ;; This should always select the element from A
                  ;; rather than B (the smaller of the two)
                  if KEYWORD_SET(keep_order) AND kind THEN return, (ui(wh[(remove_doublon_tab(ido))[0:count-1]]) < ui(wh[(remove_doublon_tab(ido))[0:count-1]]+1))
                  if kind then return, (ui(wh) < ui(wh+1))                  
                  if KEYWORD_SET(keep_order) THEN return, uu(wh[(remove_doublon_tab(ido))[0:count-1]])
                  return, uu(wh)
              endif
